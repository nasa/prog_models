...
# TODO: adjust tests for new trajectory generation structure 
class TestUAVGen(unittest.TestCase):
    ...
    def test_reference_trajectory_generation(self):
        ...
        with self.assertRaises(ProgModelInputException):
            ...
            ref_traj = traj_gen(waypoints={'flight_plan': [1, 2, 3]}, vehicle=vehicle)
        with self.assertRaises(ProgModelInputException):
            # Wrong waypoint format
            ref_traj = traj_gen(waypoints={'flight_plan': np.array([1, 2, 3])}, vehicle=vehicle)
        with self.assertRaises(ProgModelInputException):
            # Wrong format of waypoint information provided
            ref_traj = traj_gen(waypoints={'flight_plan': {'lat_deg': [1, 2, 3], 'lon_deg': [1, 2, 3], 'alt_m': [1, 2, 3]}}, vehicle=vehicle)
        with self.assertRaises(ProgModelInputException):
            # Missing waypoint information
            ref_traj = traj_gen(waypoints={'flight_plan': {'lat_deg': np.array([1, 2, 3])}}, vehicle=vehicle)
        with self.assertRaises(ProgModelInputException):
            # Missing waypoint information
            ref_traj = traj_gen(waypoints={'flight_plan': {'lon_deg': np.array([1, 2, 3]), 'alt_ft': [1, 2, 3]}}, vehicle=vehicle)
        with self.assertRaises(ProgModelInputException):
            # Missing waypoint information
            ref_traj = traj_gen(waypoints={'flight_plan': {'lat_rad': np.array([1, 2, 3]), 'alt_m': np.array([1, 2, 3])}}, vehicle=vehicle)
        with self.assertRaises(ProgModelInputException):
            # Wrong units for waypoints
            ref_traj = traj_gen(waypoints={'flight_plan': {'lat_nounit': np.array([1, 2, 3]), 'lat_nounit': np.array([1, 2, 3]), 'alt_nounit': np.array([1, 2, 3])}}, vehicle=vehicle)
        with self.assertRaises(ProgModelInputException):
            # ETAs input incorrectly
            ref_traj = traj_gen(waypoints={'flight_plan': {'lat_deg': np.array([1, 2, 3]), 'lat_deg': np.array([1, 2, 3]), 'alt_deg': np.array([1, 2, 3]), 'time_unix': np.array([1, 2])}}, vehicle=vehicle)
        with self.assertRaises(ProgModelInputException):
            # Waypoints have incorrect dimensions 
            ref_traj = traj_gen(waypoints={'flight_plan': {'lat_deg': np.array([1, 2, 3]), 'lat_deg': np.array([1, 2]), 'alt_deg': np.array([1, 2, 3]), 'time_unix': np.array([1, 2])}}, vehicle=vehicle)
