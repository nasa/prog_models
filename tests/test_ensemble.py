...
class TestEnsemble(unittest.TestCase):
    ...
    def test_two_models_identical(self):
        ...
        x1 = em.next_state(x0, u, 1)
        # m would give 2, m2 would give 4
        self.assertEqual(x1['x1'], 3)
    ...
    def test_two_models_different(self):
        ...
        self.assertFalse(em.threshold_met(x1)['x1 == 10'])
        self.assertFalse(em.threshold_met(x1)['x2 == 5'])

        # Transition again
        x2 = em.next_state(x1, u, 2)

        # Threshold met - should be combination of the two
        # x1 == 3, x2 == 6
        self.assertFalse(em.threshold_met(x2)['x1 == 10'])
        self.assertTrue(em.threshold_met(x2)['x2 == 5'])
    ...
    def test_two_models_alt_aggrigation(self):
        ...
        x0 = em.initialize()
        self.assertEqual(x0['x1'], 2)

        # Same with state transition
        u = em.InputContainer({'u1': 1})
        x1 = em.next_state(x0, u, 1)
        # m would give 3, m2 would give 5
        self.assertEqual(x1['x1'], 5)
