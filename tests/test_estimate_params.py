...
class TestEstimateParams(unittest.TestCase):
    def test_estimate_params_works(self):
        ...
        m.estimate_params(data, keys, bounds=((0, 4), (20, 42), (-20, -5)))
        tracker1 = m.calc_error(results.times, results.inputs, results.outputs)

        m.parameters['thrower_height'] = 1.5
        m.parameters['throwing_speed'] = 25
        m.parameters['g'] = -8.5

        m.estimate_params(data, keys, bounds=((-40, 40), (-40, 40), (-40, 40)))
        tracker2 = m.calc_error(results.times, results.inputs, results.outputs)
        ...
        m.estimate_params(data, keys, bounds=((0, 4), (20, 41.99), (-20, -5)))
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        m.estimate_params(data, keys, bounds=((0, 4), (20, 42), (-20, 10)))
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
...
# Testing features where keys are not parameters in the model
    ...
    def test_parameters(self):
        ...
        m.parameters['thrower_height'] = 1.2
        m.parameters['throwing_speed'] = 25
        m.parameters['g'] = -8.5

        m.estimate_params(data, keys, bounds=bound, method='Nelder-Mead', options={'maxiter':1000})
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        m.estimate_params(data, keys, bounds=bound, method='CG', options={'maxfun': 1000, 'disp': False})
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        if any(abs(m.parameters[key] - gt[key]) > 0.02 for key in keys):
    ...
    def test_compare_two_models(self):
        ...
        m.estimate_params(data, keys, bounds=bound, method='Powell', options={'maxiter': 50})
        # for key in keys:
        #     self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        self.assertNotAlmostEqual(m.calc_error(results.times, results.inputs, results.outputs),
                                    m1.calc_error(results.times, results.inputs, results.outputs), 1)
