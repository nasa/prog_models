...
from prog_models.models import ThrownObject, BatteryCircuit
...
def run_example():
    ...
    print('Generating data...')
    batt = BatteryCircuit()
    future_loading_eqns = [lambda t, x=None: batt.InputContainer({'i': 1+2*load}) for load in range(4)]
    ...
    def future_loading(t, x=None):
        return batt.InputContainer({'i':4})

    def future_loading2(t, x = None):
        nonlocal t_counter, x_counter
        z = batt.output(x_counter)
        z = m_batt.InputContainer({'i': 4, 't_t-1': z['t'], 'v_t-1': z['v'], 'dt': t - t_counter})
        x_counter = batt.next_state(x_counter, future_loading(t), t - t_counter)
        t_counter = t
        return z

    # Use new dt, not used in training
    data = batt.simulate_to_threshold(future_loading, dt=3.5, save_freq=3.5)
    results = m_batt.simulate_to(data.times[-1], future_loading2, dt=3.5, save_freq=3.5)
