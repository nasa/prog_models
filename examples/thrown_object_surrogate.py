...
def run_example(): 
    ...
    # Calculate error:
    rmse_impact = math.sqrt(sum([((impact_hf[iter] - impact_dmd[iter])**2)/len(impact_dmd) for iter in range(min(len(impact_dmd),len(impact_hf)))]))
    rmse_falling = math.sqrt(sum([((falling_hf[iter] - falling_dmd[iter])**2)/len(falling_dmd) for iter in range(min(len(falling_dmd),len(falling_hf)))]))
    rmse_position = math.sqrt(sum([((position_hf[iter] - position_dmd[iter])**2)/len(position_dmd) for iter in range(min(len(position_dmd),len(position_hf)))]))
    rmse_velocity = math.sqrt(sum([((velocity_hf[iter] - velocity_dmd[iter])**2)/len(velocity_dmd) for iter in range(min(len(velocity_dmd),len(velocity_hf)))]))    
