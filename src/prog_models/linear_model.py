...
class LinearModel(PrognosticsModel, ABC):
    ...
    def _propertyCheck(self, rowsCount, colsCount, notes):
        ...
        if not isinstance(matrix, np.ndarray):
            raise TypeError("TypeError: Matrix type check failed: @property {} dimensions is not of type list or NumPy array.".format(notes[0]))
        ...
        if (matrixShape[0] != rowsCount or # check matrix is 2 dimensional, correspond to rows count
            ...
            raise AttributeError("AttributeError: Matrix size check failed: @property {} dimensions improperly formed along {} x {}.".format(notes[0],notes[1],notes[2]))
