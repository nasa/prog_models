...
class DMDModel_dx(BatteryElectroChemEOD): #(BatteryElectroChemEODEOL):
    """
    For details, see BatteryElectroChemEOD class. 

    This class is the same, except for the next_state method, which defines the next state of the battery model based on 
    a linear approximation and Dynamic Mode Decomposition (instead of the dx method, which solves the ODE of the 
    ...
    def next_state(self, x, u, dt):
        # Define DMD state transition matrix 
        A = self.mat_DMD

        # Set-up DMD problem of the form X_next = A*X_now
            # X_now must have the following format of battery model states augmented with voltage and SOC at the current time: 
                # 1) V0, 2) Vsn, 3) Vsp, 4) tb, 5) qpS, 6) qpB, 7) qnS, 8) qnB, 9) voltage, 10) SOC, 11) current
        initial_state = np.array([[x['Vo']],[x['Vsn']],[x['Vsp']],[x['tb']],[x['qpS']],[x['qpB']],[x['qnS']],[x['qnB']]])
        SOC_0 = np.array([self.event_state(x)['EOD']]) 
        current_0 = np.array([u['i']])
        voltage_0 = np.array([self.output(x)['v']])

        X_now = np.vstack((initial_state,voltage_0,SOC_0,current_0))

        # Calculate next state using DMD
        Xprime_new = np.dot(A,X_now)

        return {
            'Vo': float(Xprime_new[0]),
            'Vsn': float(Xprime_new[1]),
            'Vsp': float(Xprime_new[2]),
            'tb': float(Xprime_new[3]),
            'qnB': float(Xprime_new[7]),
            'qnS': float(Xprime_new[6]),
...
class DMDModel(BatteryElectroChemEODEOL):
    ...
    def simulate_to(self, time, future_loading_eqn, first_output = None, **kwargs): # -> namedtuple:
        """
        Simulate DMD algorithm for a given number of seconds
        """
        # Define DMD Matrix 
        A = self.mat_DMD

        # Define time: Note: this needs to be adjusted to be more general
        t0 = 0
        t_now = t0
        dt = 1
        t_end = time
        ...
        initial_state = np.array([[x0['Vo']],[x0['Vsn']],[x0['Vsp']],[x0['tb']],[x0['qpS']],[x0['qpB']],[x0['qnS']],[x0['qnB']]])
        SOC_0 = np.array([self.event_state(x0)['EOD']]) 
        current_0 = np.array([future_loading_eqn(t0)['i']])
        voltage_0 = np.array([self.output(x0)['v']])
        ...
        # Solve DMD algorithm throughout time, for specified amount of time 
        while t_now < t_end:

            # Iterate one step forward in time 
            t_now = t_now + dt

            # Calculate next state 
            Xprime_new = np.dot(A,X_now)

            # Save and iterate forward
            state_dict = {'V0': float(Xprime_new[0]), 'Vsn': float(Xprime_new[1]), 'Vsp': float(Xprime_new[2]), 'tb': float(Xprime_new[3]), 'qpS': float(Xprime_new[4]), 'qpB': float(Xprime_new[5]), 'qnS': float(Xprime_new[6]), 'qnB': float(Xprime_new[7])}
            states_predict.append(state_dict)
            volt_now = Xprime_new[-2][0]
            voltage_predict.append(Xprime_new[-2][0])
            SOC_predict.append(Xprime_new[-1][0])
            time_predict.append(t_now)
            current_imposed.append(future_loading_eqn(t_now)['i'])
            X_now = np.vstack((Xprime_new,np.array([current_imposed[-1]])))

        return {'times': time_predict, 'SOC': SOC_predict, 'voltage': voltage_predict, 'load': current_imposed, 'states': states_predict}


    def simulate_to_threshold(self, future_loading_eqn, first_output = None, threshold_keys = None, **kwargs): # -> namedtuple:
        """
        Simulate DMD algorithm until threshold is reached
        """
        # Define DMD Matrix 
        A = self.mat_DMD
        ...
        # Solve DMD algorithm throughout time, for specified amount of time 
        while volt_now > self.parameters['VEOD']:
