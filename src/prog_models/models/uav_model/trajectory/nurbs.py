...
def evaluate(order, t, weights, waypoint_vector, basis_length=1000):
    ...
    nP, mP = waypoint_vector.shape
    if nP != 2:             waypoint_vector = waypoint_vector.T
    ...
    # -----------------------------------------
    N = np.zeros((nU, waypoint_vector.shape[1]))
    for ii in range(nU):    N[ii, :] = basisfunction(order, U[ii], t)   # first piece of basis function
    ...
    with np.errstate(divide="ignore"):  # ignore warning divide by zero
        R /= np.repeat(S.reshape((-1,1)), repeats=mP, axis=1)
    R[np.isnan(R)] = 0.
...
def generate_intermediate_points(px, py, pz, yaw, eta, weight_vector=None):
    ...
    tmpcount1 = 0  # add a temporary counter for the true weight vector
    for jj in range(len(weight_vector_new)):
        if jj % 2 == 0:
            weight_vector_new[jj] = weight_vector[tmpcount1]      # Assign pre-defined weight to the waypoint
            tmpcount1 += 1
        else:
    ...
    for ii in range(len(eta_new)):
        if ii == 0 or ii % 2 == 0:
            eta_new[ii] = eta[tmpcount1]                # Assign existing ETA
            tmpcount1 += 1
        else:
            eta_new[ii] = fictitiousETA[tmpcount2]      # Assign fictitious ETA
            tmpcount2 += 1
    ...
    for jj in range(len(yaw)):
        yaw_new[tmpcount1:tmpcount1 + 2] = yaw[jj]    # Assign new yaw value
        tmpcount1 += 2                                # this counter is
