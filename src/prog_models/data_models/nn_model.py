...
class NNStateTransitionModel(DataModel):
    """
    .. versionadded:: 1.4.0

    A State Transition Model with no :term:`event` using an Keras NN Model.
    ...
    Most users will use the :py:func:`NNStateTransitionModel.from_data` method to create a model, but the model can be created by passing in a model directly into the constructor. The NN model in this method maps from [u_t-n+1, z_t-n, ..., u_t, z_t-1] to z_t. Past :term:`input` are stored in the :term:`model` internal :term:`state`. Actual calculation of :term:`output` is performed when :py:func:`NNStateTransitionModel.output` is called. When using in simulation that may not be until the simulation results are accessed.
    ...
    See Also:
        NNStateTransitionModel.from_data
        examples.lstm_model
    """
    ...
    def __getstate__(self):
        warn("NNStateTransitionModel uses a Keras model, which does not always support pickling. We recommend that you use the keras save and load model functions instead with m.model", RuntimeWarning)
        return ((), self.parameters.data)

    def __eq__(self, other):
        # Needed because we add .model, which is not present in the parent class
        if not isinstance(other, NNStateTransitionModel):
            return False
        return super().__eq__(other) and self.model == other.model

    def initialize(self, u=None, z=None):
        """
        Initialize the model with the given inputs and outputs. For NNStateTransitionModel, the initial state is set to None. As data is added (i.e., next_state is called), the initial state is updated.
    ...
    def summary(self, file= sys.stdout, expand_nested=False, show_trainable=False):
        print('NN State Transition Model: ', file = file)
    ...
    def pre_process_data(inputs, outputs, event_states=None, t_met = None, window=10, **kwargs):
        """
        Pre-process data for the NNStateTransitionModel. This is run inside from_data to convert the data into the desired format 
    ...
    @classmethod
    def from_data(cls, base_layers, inputs, outputs, event_states = None, t_met = None, **kwargs):
        """
        Generate a NNStateTransitionModel from data

        Args:
            BaseLayerClass (List[keras.layers]): 
                Layer that constitutes the "base layers" (i.e., the state transition model layer). E.g., Dense, LSTM, or CNN
        ...
        Keyword Args:
            ...
            layers (int): 
                Number of NN layers to use. More layers can represent more complex systems, but are less efficient. Note: 2 layers is typically enough for most complex systems. Default: 1
            units (int or list[int]): 
                number of units (i.e., dimensionality of output state) used in each NN layer. Using a scalar value will use the same number of units for each layer.
        ...
        Returns:
            NNStateTransitionModel: Generated Model
        """
        ...
        params.update(NNStateTransitionModel.default_params)
        ...
        # Prepare datasets
        (u_all, z_all, es_all, t_all) = NNStateTransitionModel.pre_process_data(inputs, outputs, event_states = event_states, t_met = t_met, **params)
