...
options_surrogate = {
    ...
    'trim_data_to': 0.7
    # Value between 0 and 1 that determines the fraction of data resulting from simulate_to_threshold that is used to train DMD surrogate model
}
...
# Check serialization
print('surrogate_orig.parameters', surrogate_orig.parameters, 'new_model.parameters', new_model.parameters)
...
for i in range(min(len(surrogate_results.times), len(new_results.times))):
    print(surrogate_results.times[i], new_results.times[i])
    for key in surrogate_results.states[0].keys():
        print(key)
        print(surrogate_results.states[i][key], new_results.states[i][key], delta=3e-01)
    for key in surrogate_results.inputs[0].keys():
        print(surrogate_results.inputs[i][key], new_results.inputs[i][key], delta=3e-01)
    for key in surrogate_results.outputs[0].keys():
        print(surrogate_results.outputs[i][key], new_results.outputs[i][key], delta=3e-01)
    for key in surrogate_results.event_states[0].keys():
        print(surrogate_results.event_states[i][key], new_results.event_states[i][key], delta=3e-01)
